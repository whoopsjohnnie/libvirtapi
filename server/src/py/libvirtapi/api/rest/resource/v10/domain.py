...
from libvirtapi.dao.domain import DomainDao
...
# DomainsResource
# 
class DomainsResource(DefaultResource):

    def dao(self, driver, hypervisor):
        return DomainDao(driver, hypervisor)
    ...
    def post(self, driver, hypervisor):
        ...
        try:
            ...
            if not body:
                return self.bad("No domain XML definition found", request)

            dao = self.dao(driver, hypervisor)
            nitem = dao.createEntity(body)
            if not nitem:
                return self.error("Unable to create domain", request)

            item = self.buildDomain(
                nitem
            )
...
# DomainResource
# 
class DomainResource(DefaultResource):
    ...
    def put(self, driver, hypervisor, name):
        ...
        try:
            ...
            if not body:
                return self.bad("No domain JSON definition found", request)
